...
export const AudioComponents = () => {
  ...
  const getAllDataudio = async () => {
    try {
      const response = await axios.get("https://smartaudio.lumiconn.id/audio"); // Ganti URL sesuai dengan endpoint backend Anda
  ...
  const handleDeleteAudio = async (audioId) => {
    ...
    if (isConfirmed) {
      try {
        await axios.delete(`https://smartaudio.lumiconn.id/audio/${audioId}`);
        getAllDataudio();
      } catch (error) {
